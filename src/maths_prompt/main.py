...
def verify_ollama():
    ...
    try:
        ...
        console.print(f"[green]Ollama model {OLLAMA_MODEL} is available[/]")
    except Exception as e:
        console.print(f"[red]Ollama check failed: {e}[/]")
        console.print(f"[yellow]Make sure Ollama is running and run: ollama pull {OLLAMA_MODEL}[/]")
        raise SystemExit(1)
...
def print_config():
    ...
    table.add_row("Ollama model", OLLAMA_MODEL)
...
def main():
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            result = run_optimizer(best_prompt, best_score)
